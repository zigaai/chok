...
public class RedisService {
    ...
    // public void updateRefreshInfo(String userType, String username, String salt) {
    //     redisTemplate.execute((RedisCallback<Object>) connection -> {
    //         String userRefreshKey = RedisConstant.USER_REFRESH_TOKEN(userType, username);
    //         HashSet<String> refreshTokens = deserializeVal(connection.stringCommands().get(serializeKey(userRefreshKey)));
    //         if (CollectionUtils.isEmpty(refreshTokens)) {
    //             return null;
    //         }
    //         for (String refreshToken : refreshTokens) {
    //             byte[] rawKey = serializeKey(RedisConstant.REFRESH_TOKEN(refreshToken));
    //             byte[] bytes = connection.stringCommands().get(rawKey);
    //             PayloadDTO payload = deserializeVal(bytes);
    //             if (payload == null) {
    //                 continue;
    //             }
    //             payload.setSalt(salt);
    //             Long expire = connection.keyCommands().ttl(rawKey);
    //             if (expire == null) {
    //                 connection.keyCommands().del(rawKey);
    //                 continue;
    //             }
    //             connection.stringCommands().setEx(rawKey, expire, serializeVal(payload));
    //         }
    //         return null;
    //     });
    // }
